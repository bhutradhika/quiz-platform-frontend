...
export default function LevelContent({ category, level }: { category: string; level: string }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (cancelled) return;
      if (response.success && response.data) setQuizzes(response.data);
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (cancelled) return;
      if (response.success && response.data) setCompletedQuizIds(new Set(response.data));
    }
