...
export default function ManageQuestionsContent({ quizId }: { quizId: string }) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      if (cancelled) return;
      if (response.success && response.data) setQuiz(response.data);
      setIsLoading(false);
    }
  ...
  const handleAddQuestion = async () => {
    ...
    const questionData: QuestionRequest = {
      ...
      choices: choices.map((c, i) => ({ text: c.text, isCorrect: c.isCorrect, orderIndex: i })),
    };
