...
export default function QuizAttemptPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [quizRes, answersRes] = await Promise.all([
        ...
        attemptService.getAttemptAnswers(id),
      ]);
      if (cancelled) return;
  ...
  if (!quiz || !quiz.questions || !attemptId || isLoading) {
    return (
  ...
  const getChoiceClassName = (choiceId: string) => {
    ...
    if (choiceId === currentAnswer?.correctChoiceId) {
            ...
            <div>
              ...
              >
                {currentQuestion.choices.map((choice) => (
                  <div
                    key={choice.id}
                    className={`flex items-center gap-3 px-4 py-3 rounded-xl border-2 transition-all min-h-13 ${getChoiceClassName(choice.id)}`}
                  >
