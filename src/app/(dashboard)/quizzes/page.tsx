...
export default function QuizzesPage() {
  ...
  const getTotalQuizzes = (stats: CategoryStats) => {
    return stats.levels.reduce((sum, level) => sum + level.totalQuizzes, 0);
  };
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {categoryStats.map((stat) => {
          const totalQuizzes = getTotalQuizzes(stat);
          return (
            <Link
              key={stat.name}
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div
                  className={`w-12 h-12 rounded-xl bg-linear-to-br ${getCategoryGradient(stat.name)} flex items-center justify-center`}
                >
